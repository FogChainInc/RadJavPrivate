...
///<reference path='formatting.jump' />
...
module Formatting {
    export class Indenter implements ILineIndenationResolver  {
        ...
        constructor(
            public logger: JumpScript.ILogger,
        ...
        public GetIndentationEdits(token: TokenSpan, nextToken: TokenSpan, node: ParseNode, sameLineIndent: boolean): List_TextEditInfo {
            ...
            if (this.logger.information()) {
                for (var i = 0; i < result.count() ; i++) {
                    ...
                    this.logger.log("edit: minChar=" + edit.position + ", limChar=" + (edit.position + edit.length) + ", text=\"" + JumpScript.stringToLiteral(edit.replaceWith, 30) + "\"");
                }
            }
        ...
        private ApplyScriptBlockIndentation(languageHostIndentation: string, tree: ParseTree): void
        {
            ...
            //JumpScript: Projection snapshots not supported
        ...
        private IsMultiLineString(token: TokenSpan): boolean {
            return token.tokenID === JumpScript.TokenID.StringLiteral &&
