...
///<reference path='..\compiler\io.jump'/>
///<reference path='..\compiler\jumpscript.jump'/>
///<reference path='..\services\jumpscriptServices.jump' />
///<reference path='diff.jump'/>
...
var jumpscriptServiceFileName = filePath(IO.getExecutingFilePath()) + "jumpscriptServices.js";
var jumpscriptServiceFile = IO.readFile(jumpscriptServiceFileName);
if (typeof ActiveXObject === "function") {
    eval(jumpscriptServiceFile);
} else if (typeof require === "function") {
    var vm = require('vm');
    vm.runInThisContext(jumpscriptServiceFile, 'jumpscriptServices.js');
} else {
...
module Harness {
    ...
    /** Functionality for compiling JumpScript code */
    export module Compiler {
        /** Aggregate various writes into a single array of lines. Useful for passing to the
         *  JumpScript compiler to fill with source code or errors.
         */
        ...
        export class EmitterIOHost implements JumpScript.EmitterIOHost {
        ...
        var libFolder: string = global['WScript'] ? JumpScript.filePath(global['WScript'].ScriptFullName) : (__dirname + '/');
        export var libText = IO ? IO.readFile(libFolder + "lib.d.jump") : '';

        var stdout = new EmitterIOHost();
        var stderr = new WriterAggregator();

        export function isDeclareFile(filename: string) {
            return /\.d\.jump$/.test(filename);
        }

        export function makeDefaultCompilerForTest(c?: JumpScript.JumpScriptCompiler) {
            var compiler = c || new JumpScript.JumpScriptCompiler(stderr);
            compiler.parser.errorRecovery = true;
            compiler.settings.codeGenTarget = JumpScript.CodeGenTarget.ES5;
            ...
            JumpScript.moduleGenTarget = JumpScript.ModuleGenTarget.Synchronous;
            compiler.addUnit(Harness.Compiler.libText, "lib.d.jump", true);
            return compiler;
        }

        var compiler: JumpScript.JumpScriptCompiler;
        recreate();
        ...
        export function compile(code?: string, filename?: string) {
            if (usePull) {
                if (needsFullTypeCheck) {
                    ...
                    compiler.pullUpdateUnit(new JumpScript.StringSourceText(""), filename, true);
                    compiler.pullUpdateUnit(new JumpScript.StringSourceText(code), filename, true);
                }
            }
            else {
        ...
        export class Type {
            ...
            public compilesOk(testCode): boolean {
                var errors = null;
                compileString(testCode, 'test.jump', function (compilerResult) {
        ...
        export class TypeFactory {
            ...
            public get (code: string, target: any) {
                ...
                var errors = null;
                compileString(code, 'test.jump', function (compilerResult) {
                    errors = compilerResult.errors;
                })
                ...
                if (!usePull) {
                    ...
                    // m = 1 because the first script will always be lib.d.jump which we don't want to search.                                
                    for (var m = 1; m < compiler.scripts.members.length; m++) {
                        var script = compiler.scripts.members[m];
                        var enclosingScopeContext = JumpScript.findEnclosingScopeAt(new JumpScript.NullLogger(), <JumpScript.Script>script, new JumpScript.StringSourceText(code), 0, false);
                        var entries = new JumpScript.ScopeTraversal(compiler).getScopeEntries(enclosingScopeContext);
                    ...
                    for (var m = 0; m < compiler.scripts.members.length; m++) {
                        var script2 = <JumpScript.Script>compiler.scripts.members[m];
                        if (script2.locationInfo.filename !== 'lib.d.jump') {
                            if (targetPosition > -1) {
            ...
            private getTypeInfoName(ast : JumpScript.AST) {
                var name = '';
                switch (ast.nodeType) {
                    case JumpScript.NodeType.Name: // Type Name?
                    case JumpScript.NodeType.Null:
                    case JumpScript.NodeType.List:
                    case JumpScript.NodeType.Empty:
                    case JumpScript.NodeType.EmptyExpr:
                    case JumpScript.NodeType.Asg:
                    case JumpScript.NodeType.True:
                    case JumpScript.NodeType.False:
                    case JumpScript.NodeType.ArrayLit:
                    case JumpScript.NodeType.TypeRef:
                        break;
                    case JumpScript.NodeType.Super:
                        name = (<any>ast).text;
                        break;
                    case JumpScript.NodeType.Regex:
                        name = (<JumpScript.RegexLiteral>ast).text;
                        break;
                    case JumpScript.NodeType.QString:
                        name = (<any>ast).text;
                        break;
                    case JumpScript.NodeType.NumberLit:
                        name = (<JumpScript.NumberLiteral>ast).text;
                        break;
                    case JumpScript.NodeType.Return:
                        //name = (<JumpScript.ReturnStatement>tyInfo.ast).returnExpression.actualText; // why is this complaining?
                        break;
                    case JumpScript.NodeType.InterfaceDeclaration:
                        name = (<JumpScript.InterfaceDeclaration>ast).name.actualText;
                        break;
                    case JumpScript.NodeType.ModuleDeclaration:
                        name = (<JumpScript.ModuleDeclaration>ast).name.actualText;
                        break;
                    case JumpScript.NodeType.ClassDeclaration:
                        name = (<JumpScript.ClassDeclaration>ast).name.actualText;
                        break;
                    case JumpScript.NodeType.FuncDecl:
                        name = !(<JumpScript.FuncDecl>ast).name ? "" : (<JumpScript.FuncDecl>ast).name.actualText; // name == null for lambdas
                        break;
                    default:
        ...
        /** Generates a .d.jump file for the given code
          * @param verifyNoDeclFile pass true when the given code should generate no decl file, false otherwise
          * @param unitName add the given code under thie name, else use '0.jump'
          ...
          */
        export function generateDeclFile(code: string, verifyNoDeclFile: boolean, unitName?: string, compilationContext?: Harness.Compiler.CompilationContext, references?: JumpScript.IFileReference[]): string {
            reset();
            ...
            try {
                ...
                if (!verifyNoDeclFile) {
                    throw new Error('Compilation did not produce .d.jump files');
                }
            } finally {
        ...
        export class CompilerResult {
            ...
            constructor(public fileResults: { filename: string; file: WriterAggregator; }[], errorLines: string[], public scripts: JumpScript.Script[]) {
                var lines = [];
                ...
                for (var i = 0; i < errorLines.length; i++) {
                    if (Harness.usePull) {
                        var err = <any>errorLines[i]; // JumpScript.PullError
        ...
        /** Create a new instance of the compiler with default settings and lib.d.jump, then typecheck */
        ...
        export function addUnit(code: string, unitName?: string, isResident?: boolean, isDeclareFile?: boolean, references?: JumpScript.IFileReference[]) {
            var script: JumpScript.Script = null;
            var uName = unitName || '0' + (isDeclareFile ? '.d.jump' : '.jump');

            for (var i = 0; i < compiler.units.length; i++) {
                if (compiler.units[i].filename === uName) {
                    updateUnit(code, uName);
                    script = <JumpScript.Script>compiler.scripts.members[i];
                }
            }
            if (!script) {
        ...
        export function updateUnit(code: string, unitName: string, setRecovery?: boolean) {
            if (Harness.usePull) {
                compiler.pullUpdateUnit(new JumpScript.StringSourceText(code), unitName, setRecovery);
            } else {
                compiler.updateUnit(code, unitName, setRecovery);
            }
        }

        export function compileFile(path: string, callback: (res: CompilerResult) => void , settingsCallback?: (settings?: JumpScript.CompilationSettings) => void , context?: CompilationContext, references?: JumpScript.IFileReference[]) {
        ...
        export function compileUnit(code: string, filename: string, callback: (res: CompilerResult) => void , settingsCallback?: (settings?: JumpScript.CompilationSettings) => void , context?: CompilationContext, references?: JumpScript.IFileReference[]) {
            // not recursive
            ...
            var oldCompilerSettings = new JumpScript.CompilationSettings();
            clone(compiler.settings, oldCompilerSettings);
            var oldEmitSettings = new JumpScript.EmitOptions(compiler.settings);
            clone(compiler.emitSettings, oldEmitSettings);

            var oldModuleGenTarget = JumpScript.moduleGenTarget;

            if (settingsCallback) {
                settingsCallback(compiler.settings);
                compiler.emitSettings = new JumpScript.EmitOptions(compiler.settings);
            }
            try {
                ...
                if (settingsCallback) {
                    ...
                    JumpScript.moduleGenTarget = oldModuleGenTarget;
                }
            }
        }
        ...
        export function emit(ioHost: JumpScript.EmitterIOHost, usePullEmitter?: boolean) {
            compiler.emit(ioHost, usePullEmitter);
        }

        export function compileString(code: string, unitName: string, callback: (res: Compiler.CompilerResult) => void , context?: CompilationContext, references?: JumpScript.IFileReference[]) {
            var scripts: JumpScript.Script[] = [];
            ...
            // for single file tests just add them as using the old '0.jump' naming scheme
            var uName = context ? unitName : ((isDeclareFile) ? '0.d.jump' : '0.jump');
    ...
    export module TestCaseParser {
        ...
        export interface TestUnitData {
            ...
            references: JumpScript.IFileReference[];
        }
        ...
        export function makeUnitsFromTest(code: string, filename: string): { settings: CompilerSetting[]; testUnitData: TestUnitData[]; } {
            ...
            var refs: JumpScript.IFileReference[] = [];
            ...
            for (var i = 0; i < lines.length; i++) {
                ...
                if (isTripleSlashReference) {
                    var isRef = line.match(/reference\spath='(\w*_?\w*\.?d?\.jump)'/);
                    if (isRef) {
                        var ref = {
                            minChar: 0,
                            limChar: 0,
            ...
            currentFileName = files.length > 0 ? currentFileName : '0.jump';
    ...
    export class ScriptInfo {
        ...
        public editRanges: { length: number; editRange: JumpScript.ScriptEditRange; }[] = [];
        ...
        public editContent(minChar: number, limChar: number, newText: string) {
            ...
            this.editRanges.push({
                ...
                editRange: new JumpScript.ScriptEditRange(minChar, limChar, (limChar - minChar) + newText.length)
            });
        ...
        public getEditRangeSinceVersion(version: number): JumpScript.ScriptEditRange {
            ...
            if (initialEditRangeIndex < 0 || initialEditRangeIndex >= this.editRanges.length) {
                ...
                return JumpScript.ScriptEditRange.unknown();
            }
            ...
            return new JumpScript.ScriptEditRange(minDistFromStart, entries[0].length - minDistFromEnd, aggDelta);
        }
    }

    export class JumpScriptLS implements Services.ILanguageServiceShimHost {
        ...
        public addDefaultLibrary() {
            this.addScript("lib.d.jump", Harness.Compiler.libText, true);
        }
        ...
        public getLanguageService(): Services.ILanguageServiceShim {
            var ls = new Services.JumpScriptServicesFactory().createLanguageServiceShim(this);
            ls.refresh(true);
            this.ls = ls;
            return ls;
        }
        ...
        public parseSourceText(fileName: string, sourceText: JumpScript.ISourceText): JumpScript.Script {
            var parser = new JumpScript.Parser();
        ...
        public parseFile(fileName: string) {
            var sourceText = new JumpScript.StringSourceText(IO.readFile(fileName))
        ...
        */
        public lineColToPosition(fileName: string, line: number, col: number): number {
            ...
            return JumpScript.getPositionFromZeroBasedLineColumn(script, line - 1, col - 1);
        }
        ...
        */
        public positionToZeroBasedLineCol(fileName: string, position: number): JumpScript.ILineCol {
            var script = this.ls.languageService.getScriptAST(fileName);
            assert.notNull(script);

            var result = JumpScript.getZeroBasedLineColumnFromPosition(script, position);
    ...
    /** Runs JumpScript or Javascript code. */
...
(function (Harness) {
    ...
    (function (Compiler) {
        ...
        var libFolder = global['WScript'] ? JumpScript.filePath(global['WScript'].ScriptFullName) : (__dirname + '/');
        Compiler.libText = IO ? IO.readFile(libFolder + "lib.d.jump") : '';
        var stdout = new EmitterIOHost();
        var stderr = new WriterAggregator();
        ...
        var TypeFactory = /** @class */ (function () {
            ...
            TypeFactory.prototype.get = function (code, target) {
                ...
                if (!Harness.usePull) {
                    ...
                    for (var m = 1; m < compiler.scripts.members.length; m++) {
                        var script = compiler.scripts.members[m];
                        var enclosingScopeContext = JumpScript.findEnclosingScopeAt(new JumpScript.NullLogger(), script, new JumpScript.StringSourceText(code), 0, false);
                        var entries = new JumpScript.ScopeTraversal(compiler).getScopeEntries(enclosingScopeContext);
                        for (var i = 0; i < entries.length; i++) {
                            if (entries[i].name === targetIdentifier) {
                                matchingIdentifiers.push(new Type(entries[i].type, code, targetIdentifier));
        ...
        var CompilerResult = /** @class */ (function () {
            ...
            function CompilerResult(fileResults, errorLines, scripts) {
                ...
                for (var i = 0; i < errorLines.length; i++) {
                    if (Harness.usePull) {
                        var err = errorLines[i]; // JumpScript.PullError
                        this.errors.push(new CompilerError(err.filename, 0, 0, err.message));
                    }
                    else {
                        var match = errorLines[i].match(/([^\(]*)\((\d+),(\d+)\):\s+((.*[\s\r\n]*.*)+)\s*$/);
    ...
    var JumpScriptLS = /** @class */ (function () {
        function JumpScriptLS() {
            this.ls = null;
            this.scripts = [];
            this.maxScriptVersions = 100;
        }
        JumpScriptLS.prototype.addDefaultLibrary = function () {
            this.addScript("lib.d.jump", Harness.Compiler.libText, true);
        };
        JumpScriptLS.prototype.addFile = function (name, isResident) {
            if (isResident === void 0) { isResident = false; }
            var code = readFile(name);
            this.addScript(name, code, isResident);
        };
        JumpScriptLS.prototype.addScript = function (name, content, isResident) {
            if (isResident === void 0) { isResident = false; }
            var script = new ScriptInfo(name, content, isResident, this.maxScriptVersions);
            this.scripts.push(script);
        };
        JumpScriptLS.prototype.updateScript = function (name, content, isResident) {
            if (isResident === void 0) { isResident = false; }
            for (var i = 0; i < this.scripts.length; i++) {
                if (this.scripts[i].name == name) {
                    this.scripts[i].updateContent(content, isResident);
        ...
        JumpScriptLS.prototype.editScript = function (name, minChar, limChar, newText) {
            for (var i = 0; i < this.scripts.length; i++) {
                if (this.scripts[i].name == name) {
                    this.scripts[i].editContent(minChar, limChar, newText);
                    return;
        ...
        JumpScriptLS.prototype.getScriptContent = function (scriptIndex) {
            return this.scripts[scriptIndex].content;
        };
        //////////////////////////////////////////////////////////////////////
        // ILogger implementation
        //
        JumpScriptLS.prototype.information = function () { return false; };
        JumpScriptLS.prototype.debug = function () { return true; };
        JumpScriptLS.prototype.warning = function () { return true; };
        JumpScriptLS.prototype.error = function () { return true; };
        JumpScriptLS.prototype.fatal = function () { return true; };
        JumpScriptLS.prototype.log = function (s) {
            // For debugging...
            //IO.printLine("JumpScriptLS:" + s);
        };
        //////////////////////////////////////////////////////////////////////
        // ILanguageServiceShimHost implementation
        //
        JumpScriptLS.prototype.getCompilationSettings = function () {
            return ""; // i.e. default settings
        };
        JumpScriptLS.prototype.getScriptCount = function () {
            return this.scripts.length;
        };
        JumpScriptLS.prototype.getScriptSourceText = function (scriptIndex, start, end) {
            return this.scripts[scriptIndex].content.substring(start, end);
        };
        JumpScriptLS.prototype.getScriptSourceLength = function (scriptIndex) {
            return this.scripts[scriptIndex].content.length;
        };
        JumpScriptLS.prototype.getScriptId = function (scriptIndex) {
            return this.scripts[scriptIndex].name;
        };
        JumpScriptLS.prototype.getScriptIsResident = function (scriptIndex) {
            return this.scripts[scriptIndex].isResident;
        };
        JumpScriptLS.prototype.getScriptVersion = function (scriptIndex) {
            return this.scripts[scriptIndex].version;
        };
        JumpScriptLS.prototype.getScriptEditRangeSinceVersion = function (scriptIndex, scriptVersion) {
            var range = this.scripts[scriptIndex].getEditRangeSinceVersion(scriptVersion);
            var result = (range.minChar + "," + range.limChar + "," + range.delta);
            return result;
        };
         ...
         *  To access the non-shim (i.e. actual) language service, use the "ls.languageService" property.
         */
        JumpScriptLS.prototype.getLanguageService = function () {
            var ls = new Services.JumpScriptServicesFactory().createLanguageServiceShim(this);
            ls.refresh(true);
            this.ls = ls;
            return ls;
        ...
        /** Parse file given its source text */
        JumpScriptLS.prototype.parseSourceText = function (fileName, sourceText) {
            var parser = new JumpScript.Parser();
            parser.setErrorRecovery(null);
        ...
        /** Parse a file on disk given its filename */
        JumpScriptLS.prototype.parseFile = function (fileName) {
            var sourceText = new JumpScript.StringSourceText(IO.readFile(fileName));
            return this.parseSourceText(fileName, sourceText);
        };
        /**
         ...
         * @param col 1 based index
        */
        JumpScriptLS.prototype.lineColToPosition = function (fileName, line, col) {
            var script = this.ls.languageService.getScriptAST(fileName);
            assert.notNull(script);
            assert.is(line >= 1);
            assert.is(col >= 1);
         ...
         * @param col 0 based index
        */
        JumpScriptLS.prototype.positionToZeroBasedLineCol = function (fileName, position) {
            var script = this.ls.languageService.getScriptAST(fileName);
            assert.notNull(script);
            var result = JumpScript.getZeroBasedLineColumnFromPosition(script, position);
            assert.is(result.line >= 0);
        ...
        /** Verify that applying edits to sourceFileName result in the content of the file baselineFileName */
        JumpScriptLS.prototype.checkEdits = function (sourceFileName, baselineFileName, edits) {
            var script = readFile(sourceFileName);
            var formattedScript = this.applyEdits(script, edits);
            var baseline = readFile(baselineFileName);
            assert.noDiff(formattedScript, baseline);
        ...
        /** Apply an array of text edits to a string, and return the resulting string. */
        JumpScriptLS.prototype.applyEdits = function (content, edits) {
            var result = content;
            edits = this.normalizeEdits(edits);
            for (var i = edits.length - 1; i >= 0; i--) {
                var edit = edits[i];
        ...
        /** Normalize an array of edits by removing overlapping entries and sorting entries on the minChar position. */
        JumpScriptLS.prototype.normalizeEdits = function (edits) {
            var result = [];
        ...
        JumpScriptLS.prototype.getHostSettings = function () {
            return JSON.stringify({ usePullLanguageService: Harness.usePull });
        };
        return JumpScriptLS;
    }());
    Harness.JumpScriptLS = JumpScriptLS;
    // Describe/it definitions
