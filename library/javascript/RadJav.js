    ...
    var Theme = /** @class */ (function () {
            ...
            try {
                if (RadJav.currentTheme.themeObjects[file] != null) {
                    if (RadJav.currentTheme.themeObjects[file][event] != null) {
                        return RadJav.currentTheme.themeObjects[file][event].apply(RadJav.currentTheme.themeObjects[file], args);
                    }
                    else {
                        if (file.indexOf("GUI") > -1) {
                            var tempfile = "RadJav.GUI.GObject";
                            if (RadJav.currentTheme.themeObjects[tempfile][event] != null) {
                                return RadJav.currentTheme.themeObjects[tempfile][event].apply(RadJav.currentTheme.themeObjects[tempfile], args);
                            }
                        }
                    }
                }
        ...
        Theme.prototype.eventSync = function (file, event) {
            ...
            try {
                if (RadJav.currentTheme.themeObjects[file] != null) {
                    if (RadJav.currentTheme.themeObjects[file][event] != null) {
                        result = RadJav.currentTheme.themeObjects[file][event].apply(RadJav.currentTheme.themeObjects[file], args);
                    }
                    else {
                        if (file.indexOf("GUI") > -1) {
                            var tempfile = "RadJav.GUI.GObject";
                            if (RadJav.currentTheme.themeObjects[tempfile][event] != null) {
                                result = RadJav.currentTheme.themeObjects[tempfile][event].apply(RadJav.currentTheme.themeObjects[tempfile], args);
                            }
                        }
                    }
                }
