...
###   This script attempts to download the signature file SHA256SUMS.asc from xrjv1.org
...
WORKINGDIR="/tmp/xrjv1_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://xrjv1.org/bin/"
VERSIONPREFIX="xrjv1-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'xrjv1-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the XRJV1 Core binary release signing key installed?"
   fi
