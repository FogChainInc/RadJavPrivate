#!/bin/bash
# Copyright (c) 2013 The XRJV1 Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete XRJV1 database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no XRJV1 datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old XRJV1 datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected XRJV1 0.7 datadir."
    ;;
  3)
    echo "Detected XRJV1 pre-0.8 datadir."
    ;;
  4)
    echo "Detected XRJV1 0.8 datadir."
    ;;
esac
