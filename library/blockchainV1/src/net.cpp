...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (XRJV1 users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "RadJav Blockchain V1 " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_XRJ("Unable to bind to %s on this computer. %s is probably already running."), addrBind.ToString(), _XRJ(PACKAGE_NAME));
        else
            strError = strprintf(_XRJ("Unable to bind to %s on this computer (bind returned error %s)"), addrBind.ToString(), NetworkErrorString(nErr));
        LogPrintf("%s\n", strError);
    ...
    {
        strError = strprintf(_XRJ("Error: Listening for incoming connections failed (listen returned error %s)"), NetworkErrorString(WSAGetLastError()));
        LogPrintf("%s\n", strError);
        CloseSocket(hListenSocket);
        return false;
    }
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    if (clientInterface)
        clientInterface->InitMessage(_XRJ("Loading addresses..."));
    ...
    if (clientInterface)
        clientInterface->InitMessage(_XRJ("Loading banlist..."));
    ...
    uiInterface.InitMessage(_XRJ("Starting network threads..."));
