...
#if defined(NDEBUG)
# error "RadJav Blockchain V1 cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "XRJV1 Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s XRJ additional fees, %d delta bytes\n",
...
bool AbortNode(const std::string& strMessage, const std::string& userMessage="")
{
    ...
    uiInterface.ThreadSafeMessageBox(
        userMessage.empty() ? _XRJ("Error: A fatal internal error occurred, see debug.log for details") : userMessage,
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    LogPrint("bench", "    - Callbacks: %.2fms [%.2fs]\n", 0.001 * (nTime6 - nTime5), nTimeCallbacks * 0.000001);
		...
		if (RadJAV::RadJav::javascriptEngine != NULL)
			RadJAV::RadJav::javascriptEngine->blockchainEvent("connectBlock", "int", &pindex->nHeight);
	#endif
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            if (state == THRESHOLD_ACTIVE || state == THRESHOLD_LOCKED_IN) {
                if (state == THRESHOLD_ACTIVE) {
                    std::string strWarning = strprintf(_XRJ("Warning: unknown new rules activated (versionbit %i)"), bit);
        ...
        if (nUpgraded > 100/2)
        {
            std::string strWarning = _XRJ("Warning: Unknown block versions being mined! It's possible unknown rules are in effect");
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
        return AbortNode("Disk space is low!", _XRJ("Error: Disk space is low!"));
...
CVerifyDB::CVerifyDB()
{
    uiInterface.ShowProgress(_XRJ("Verifying blocks..."), 0);
}
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        uiInterface.ShowProgress(_XRJ("Verifying blocks..."), percentageDone);
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            boost::this_thread::interruption_point();
            uiInterface.ShowProgress(_XRJ("Verifying blocks..."), std::max(1, std::min(99, 100 - (int)(((double)(chainActive.Height() - pindex->nHeight)) / (double)nCheckDepth * 50))));
