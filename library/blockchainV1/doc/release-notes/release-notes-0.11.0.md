XRJV1 Core version 0.11.0 is now available from:

  <https://xrjv1.org/bin/xrjv1-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer XRJ/kB fee to be
...
For example, add the following to `xrjv1.conf`:
...
related to the blockchain in the xrjv1 system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows XRJV1 Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that XRJV1 Core will 
...
For an example script see [xrjv1-submittx](https://github.com/laanwj/xrjv1-submittx).
...
- #5334 `cf87536` libxrjv1consensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new xrjv1-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only XRJV1AmountField
- #5753 `0f44672` Add xrjv1 logo to about screen
...
- #5809 `46bfbe7` Add xrjv1-cli man page
...
- #5849 `41259ca` contrib/init/xrjv1d.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename XRJV1-Qt.app to XRJV1-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for xrjv1-tx)
