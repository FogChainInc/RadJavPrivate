...
*Setup instructions for a Gitian build of XRJV1 Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the XRJV1
...
- [Building XRJV1 Core](#building-xrjv1-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-xrjv1-host/README.md).
...
Clone the git repositories for xrjv1 and Gitian.
...
in the xrjv1 repository under 'Fetch and create inputs' to install sources which require
...
To build XRJV1 Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the xrjv1 repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xrjv1/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/xrjv1/xrjv1
...
For example:
```bash
URL=https://github.com/laanwj/xrjv1.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit xrjv1=${COMMIT} --url xrjv1=${URL} ../xrjv1/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xrjv1=${COMMIT} --url xrjv1=${URL} ../xrjv1/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xrjv1=${COMMIT} --url xrjv1=${URL} ../xrjv1/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the xrjv1 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../xrjv1/contrib/gitian-descriptors/*|sort|uniq )
...
XRJPATH=/some/root/path/xrjv1
SIGPATH=/some/root/path/xrjv1-detached-sigs

./bin/gbuild --url xrjv1=${XRJPATH},signature=${SIGPATH} ../xrjv1/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xrjv1-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xrjv1-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/xrjv1-osx-build.assert
```
...
[xrjv1-core/gitian.sigs](https://github.com/xrjv1-core/gitian.sigs/) repository, or if that's not possible create a pull
