...
BoostServer::BoostServer(int port, int type) {
	(void)port; //not used
	(void)type; //not used
...
void BoostServer::listen() {
	...
	std::make_shared<listener>(*io_context_, tcp::endpoint{ address, port })->run();

};

/// Serve any GET or POST requests.
void BoostServer::serve() {
