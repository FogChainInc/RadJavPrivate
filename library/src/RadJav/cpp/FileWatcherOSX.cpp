...
{	
	...
	void FileWatcherOSX::update()
	{
		...
		for(; iter != end; ++iter)
		{
			...
			while((nev = kevent(m_descriptor, (KEvent*)&(watch->mChangeList), watch->mChangeListCount + 1, &event, 1, &m_timeout)) != 0)
			{
				if(nev == -1)
					perror("kevent");
				else if ( nev > 0 )
...
*/
		...
		WatchStruct* watch = new WatchStruct(++m_lastWatchID, directory, watcher);
		mWatches.insert(std::make_pair(m_lastWatchID, watch));
		return m_lastWatchID;
	}
