; Thank you to the Electrum team for this code.
; https://github.com/spesmilo/electrum/blob/master/contrib/build-wine/electrum.nsi
...
;Variables
  ...
  !define PRODUCT_PUBLISHER "FogChain, Corp."
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "${PRODUCT_ARCH}\build\RadJavVM-Windows-${PRODUCT_ARCH}-${PRODUCT_VERSION}.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\..\assets\radjav.ico"
...
Section
  ...
  ReadRegDword $R1 HKLM "HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\VisualStudio\14.0\VC\Runtimes\${PRODUCT_ARCH}" "Installed"

  ;${if} $R1 != "1"
    ExecWait "$INSTDIR\VC_redist.${PRODUCT_ARCH}.exe"
  ;${else}
    ;DetailPrint "Found Visual C++ 2017 ${PRODUCT_ARCH} Redistributable..."
  ;${EndIf}
  ...
  File /r "${PRODUCT_ARCH}\build\RadJavVM\*.*"
  File "..\..\..\assets\radjav.ico"
  ...
  DetailPrint "Registering file extension XRJ..."
  ${registerExtension} "$INSTDIR\RadJavVM.exe" ".xrj" "RadJav App"
  ...
  ;CreateShortCut "$DESKTOP\RadJav Example Browser.lnk" "$INSTDIR\RadJavVM.exe" ".\examples\exampleBrowser.xrj"

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\RadJav Example Browser.lnk" "$INSTDIR\RadJavVM.exe" ".\examples\exampleBrowser.xrj" "$INSTDIR\RadJavVM.exe" 0
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\radjav.ico"
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\xrjv1"
  DeleteRegKey HKCU "Software\${PRODUCT_NAME}"
  DeleteRegKey HKLM "Software\${PRODUCT_NAME}"
  DeleteRegKey HKCU "${PRODUCT_UNINST_KEY}"

  ${unregisterExtension} ".xrj" "RadJav App"
SectionEnd
